                ...
                <div class="row">
                    <div class="form-group col-md-4 required">
                        ...
                        <input type="name" name="name" class="form-control form-control-sm  @error('name') is-invalid @enderror"
                            id="name" placeholder="Enter Name" value="{{ old('name') ?? $record->name }}" required>
                    ...
                    <div class="form-group col-md-4 required">
                        ...
                        <input type="email" name="email" class="form-control form-control-sm @error('email') is-invalid @enderror"
                            id="email" placeholder="Enter email" value="{{ old('email') ?? $record->email }}" required>
                        <x-form-error key="email" />
                    </div>
                    @else
                    <div class="form-group col-md-4">
                        <label for="email">Email</label>
                        <input type="text" disabled class="form-control form-control-sm" placeholder="Email"
                            value="{{ $record->email }}" />
                    </div>
                    @endrole
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="roles[]" class="form-control form-control-sm select2bs4 @error('roles') is-invalid @enderror"
                            ...
                            <option value="{{ $role->id }}" {{ $record->hasRole($role->id) ? 'selected' : '' }}>
                                {{ $role->name }}</option>
                            @endforeach
                        </select>
